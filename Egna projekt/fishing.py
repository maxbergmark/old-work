...
def difcolor(target, image, size):
    ...
    poss = [[(1000+i,500+j) for i in range(0,2000,diff)] for j in range(0,1000,diff)]
    ...
    while True:
        ...
        for i in range(-1, 2):
            for j in range(-1, 2):
                ...
                if checkCol(colors[minI+1*i][minJ+1*j], tempTuple) > 8000:
                    diff += 1
