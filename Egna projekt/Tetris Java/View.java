...
public class View extends JPanel implements Runnable{
	...
	public View (Tetris tetrisIn, int frameTimeIn) {
		tetris = tetrisIn;
		tetrisFrame = new JFrame("Tetris " + tetris.randSeed + ".0");
	...
	public void paint(Graphics g) {
		...
		for (int x = 0; x < xsize; x++) {
			for (int y = 1; y < ysize; y++) {
				g2D.setColor(colors[tetris.getGrid()[y][x]]);
		...
		String blockFreq = "";
		String lineFreq = "";
		String blocksPerLine = "";
		String randLinesFreq = "";
		if(nanoTime > 3) {
			blockFreq = String.valueOf(round((double)(1000000000*totalBlocks/(double)(nanoTime)), 2));
			lineFreq = String.valueOf(round((double)(1000000000*score/(double)(nanoTime)), 2));
			blocksPerLine = String.valueOf(round(((double)(totalBlocks)/(double)(Math.max(randLines, 1))), 2));
			randLinesFreq = String.valueOf(round((double)(1000000000*randLines/(double)(nanoTime)), 2));
		}
