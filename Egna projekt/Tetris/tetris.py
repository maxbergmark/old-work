...
class App:
    ...
    def playSelf(self):
        ...
        nextMinHeight = self.height
        nextMinPlace = 0
        nextMinRot = 0
        nextMinHoles = self.height*self.width
...
####
        ...
        for e in range(4):
            ...
            if (self.mainShape in ['line', 'squiggly', 'revSquiggly'] and e < 2) or (self.mainShape in ['square'] and e < 1) or (self.mainShape in ['lBlock', 'revlBlock', 'tBlock']):
                ...
                while self.calcLeftRight(temp)[0] > 0:
                    temp = self.translateShape([0, -1], temp)
                    translation -= 1
                while True:
                    matrix = self.places[:]
    ...
    ##
                        ...
                        for k in range(4):
                            nextTranslation = 0
                            nextTemp = self.translateShape([1, 0], [nextFirstRot, nextSecondRot, nextThirdRot, nextFourthRot][k])
                            if (self.next in ['line', 'squiggly', 'revSquiggly'] and k < 2) or (self.next in ['square'] and k < 1) or (self.next in ['lBlock', 'revlBlock', 'tBlock']):
                                ...
                                while True:
                                    ...
                                    if not self.checkIntersect(nextTemp[:]):
                                        ...
                                        waysChecked += 1
                                        if nextMinHoles >= nextTempHoles:
                                            if nextMinHeight >= nextTempHeight and nextcLR[0] >= 0:
                                                nextMinHoles = nextTempHoles
                                                nextMinHeight = nextTempHeight
                                                nextMinPlace = nextTranslation
                                                nextMinRot = k
                                                firstMinRot = e
                                                firstMinPlace = translation
    ...
    ##
                    ...
                    if self.calcLeftRight(temp)[1] > self.width-1:
...
######
                                ...
                                if (self.next in ['line', 'squiggly', 'revSquiggly'] and k < 2) or (self.next in ['square'] and k < 1) or (self.next in ['lBlock', 'revlBlock', 'tBlock']):
                                    ...
                                    while True:
                                        ...
                                        if not self.checkIntersect(nextTemp[:]):
                                            ...
                                            waysChecked += 1
                                            if nextMinHoles >= self.calcHoles(nextRowedMatrix[:]):
                                                if nextMinHeight >= self.calcHeight(nextRowedMatrix[:]) and nextcLR[0] >= 0:
                                                    nextMinHoles = self.calcHoles(nextRowedMatrix[:])
                                                    nextMinHeight = self.calcHeight(nextRowedMatrix[2:self.height+1])
                                                    nextMinPlace = nextTranslation
                                                    nextMinRot = k
                                                    firstMinRot = e
                                                    firstMinPlace = translation
                                                    nextMinRowed = nextRowedMatrix[:]
                                                    if self.holdAble:
                                                        self.holdObject()
        ...
        self.calcCenter()
        self.drawMain()
        for i in range(firstMinRot+1):
            self.rotateRight()
        self.eraseMain()
        for i in range(abs(firstMinPlace)):
            if self.checkRight() and self.checkLeft():
                
                self.translateShape([0, int(firstMinPlace/abs(firstMinPlace))])
        self.calcCenter()
        self.drawMain()
        if self.dropHard:
    ...
    def spawn(self):
        ...
        self.drawMain()
        self.totalBlocks += 1;
        print(self.totalBlocks/(time.clock()-self.timeStarted))
