...
public class View extends JPanel{
	...
	public View(Model m, int xs, int ys) {
    	...
    	testframe.setPreferredSize(new Dimension(xsize+16, ysize+38));
    	//testframe.add(this);
	...
	public void paint(Graphics g) {
		...
		for (int i = 0; i < testmodel.getPN(); i++) {
			if (!testmodel.getParticle(i).getFrozen()) {
				int tempx = (int)(testmodel.getParticle(i).getXRel());
				int tempy = (int)(testmodel.getParticle(i).getYRel());
				int tempr = (int)testmodel.getParticle(i).getR();
				g2D.setColor(testmodel.getParticle(i).getColor());
				g2D.fill(new Ellipse2D.Double(tempx-tempr, tempy-tempr, 2*tempr, 2*tempr));
			}
		}
