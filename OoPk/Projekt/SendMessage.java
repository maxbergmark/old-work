...
public class SendMessage implements Runnable { //Klass för att skicka filer.
    ...
    try {
      while (true) {
        ...
        try{
          ...
          String xmlMessage;
          if (!message.getIfXML()) {
            if (message.getText().equals("<keyRequest>")) { //if-sats som fångar keyrequest.
              window.startTimer();
            }
            if (message.getText().startsWith("<filerequest>")) { //if-sats som fångar filerequest.
              window.startFileTimer();
            }
