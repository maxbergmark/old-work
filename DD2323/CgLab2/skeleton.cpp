	...
	Intersection& closestIntersection) {
		...
		for (int i = 0; i < triangles.size(); i++) {
			...
			if (fac != 0) {
				...
				if (dist > 0 & (dist < closestIntersection.distance | true) & i != closestIntersection.originTriangle) {
...
void Draw()
{
	...
	for (int k = 0; k < SCREEN_HEIGHT/10; k++) {
		for (int j = 0; j < SCREEN_WIDTH/10; j++) {
			PutPixelSDL( screen, j, k, vec3(1, 0, 0) );
		}
	}
	...
	for (int y = 0; y < SCREEN_HEIGHT; y++) {
		for (int x = 0; x < SCREEN_WIDTH; x++) {
			...
			if (test && (closestIntersection.onEdge)) {
				...
				for (int a = 1-AAFACTOR; a < AAFACTOR; a++) {
					for (int b = 1-AAFACTOR; b < AAFACTOR; b++) {
						...
						if (test & (a != 0 && b != 0) & EDGESHOW) {
							...
							//color += 1.0f/(2*AAFACTOR-1)/(2*AAFACTOR-1)*(DirectLight(closestIntersection)+indirectLight);
							...
							//color += 1.0f/(2*AAFACTOR-1)/(2*AAFACTOR-1)*triangles[closestIntersection.triangleIndex].color;
						} else if (test & EDGESHOW) {
							color += 1.0f/(2*AAFACTOR-1)/(2*AAFACTOR-1)*triangles[closestIntersection.triangleIndex].color*(dirLight+indirectLight);
							//color += 1.0f/(2*AAFACTOR-1)/(2*AAFACTOR-1)*(DirectLight(closestIntersection)+indirectLight);
							//color += 1.0f/(2*AAFACTOR-1)/(2*AAFACTOR-1)*triangles[closestIntersection.triangleIndex].color;
						}
				...
				color = triangles[closestIntersection.triangleIndex].color*(dirLight+indirectLight);
