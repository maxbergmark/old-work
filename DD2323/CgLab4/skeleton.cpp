...
vec3 DirectLight(Intersection& i) {
	...
	vec3 D = lightColor*std::fmax(glm::dot(glm::normalize(r), triangles[i.triangleIndex].normal), 0.0f)/(4*3.14159265358979f*glm::dot(r, r));
		...
		for (int i = 0; i < triangles.size(); i++) {
			...
			if (fac != 0) {
				...
				if (dist > 0 & dist < closestIntersection.distance && i != closestIntersection.originTriangle) {
					...
					if ((x1 >= -0.02f && x2 >= -0.02f && x1+x2 < 1.02f) && (round(50*(x1+x2))/50 == 1.0f || round(50*x1) == 0 || round(50*x2) == 0)) {
